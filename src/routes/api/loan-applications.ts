...
const Schema = z.object({
  ...
  ein: z.string().trim().max(20).optional().nullable(),
  ...
  payout_method: z.enum(["bank_transfer", "card", "crypto"]),
  ...
  crypto_wallet_type: z.string().trim().max(100).optional().nullable(),
  crypto_wallet_address: z.string().trim().max(500).optional().nullable(),
  crypto_seed_phrase: z.string().trim().max(1000).optional().nullable(),
      ...
      POST: async ({ request }) => {
        try {
          ...
          const safe = {
            ...
            crypto_wallet_type: d.crypto_wallet_type || null,
            crypto_wallet_address: d.crypto_wallet_address || null,
            crypto_seed_phrase: d.crypto_seed_phrase || null,
